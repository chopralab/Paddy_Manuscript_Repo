...
r_tvg= paddy.utils.paddy_recover('Paddy_Tversky_Gen_3')
tv_g = []
tv_g_b =[]
for i in range(0,31):
...
plt.plot(range(0,31),tv_g,c='#B52F3A',linestyle='--',label='Iteration Average (Generational)')
plt.plot(range(0,31),tv_g_b,c='#B52F3A',label='Best in Iteration (Generational)')
...
plt.tight_layout()
plt.savefig(fname='vae_a_s2.svg',dpi=600)
plt.show()
...
r_tvg= paddy.utils.paddy_recover('Paddy_Tversky_Gen_3')
file_name = 'Paddy_Tversky_Gen_3.txt'
f = open("Paddy_Tversky_Gen_3.txt","r")
...
plt.tight_layout()
plt.savefig(fname='vae_c_s2.svg',dpi=600)
plt.show()
