...
class JTNNVAE(nn.Module):
    ...
    def decode(self, x_tree_vecs, x_mol_vecs, prob_decode):
        ...
        ts = time.time()
        cur_mol,_ = self.dfs_assemble(tree_mess,ts, x_mol_vecs, pred_nodes, cur_mol, global_amap, [], pred_root, None, prob_decode, check_aroma=True)
        #print('i want to die')
        if cur_mol == 'kill me':
            return 'kill me'
        if cur_mol is None:
            ...
            global_amap = [{}] + [{} for node in pred_nodes]
            global_amap[1] = {atom.GetIdx():atom.GetIdx() for atom in cur_mol.GetAtoms()}
            cur_mol,pre_mol = self.dfs_assemble(tree_mess,ts, x_mol_vecs, pred_nodes, cur_mol, global_amap, [], pred_root, None, prob_decode, check_aroma=False)
    ...
    def dfs_assemble(self, y_tree_mess, ts, x_mol_vecs, all_nodes, cur_mol, global_amap, fa_amap, cur_node, fa_node, prob_decode, check_aroma):
        if time.time() > ts+10:
                #print('im fina die')
		return 'kill me', 'kill me'
        ...
        l = 0
        #print('xrange')
        #print(len(xrange(cand_idx.numel())))
        for i in xrange(cand_idx.numel()):
            ...
            for nei_node in children:
                ...
                tmp_mol, tmp_mol2 = self.dfs_assemble(y_tree_mess, ts, x_mol_vecs, all_nodes, cur_mol, new_global_amap, pred_amap, nei_node, cur_node, prob_decode, check_aroma)
                if tmp_mol == 'kill me':
			return 'kill me', 'kill me'
                if tmp_mol is None: 
                    has_error = True
